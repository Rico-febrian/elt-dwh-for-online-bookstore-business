import luigi
import pandas as pd
import logging
import time
import os
from datetime import datetime
from .utils_function.db_connector import db_connector
from .utils_function.read_sql import read_sql_file
from dotenv import load_dotenv


# Load environment variables from .env file
load_dotenv()

# Define DIR
DIR_ROOT_PROJECT = os.getenv("DIR_ROOT_PROJECT")
DIR_TEMP_LOG = os.getenv("DIR_TEMP_LOG")
DIR_TEMP_DATA = os.getenv("DIR_TEMP_DATA")
DIR_EXTRACT_QUERY = os.getenv("DIR_EXTRACT_QUERY")
DIR_LOG = os.getenv("DIR_LOG")


class Extract(luigi.Task):
    
    # Define tables to be extracted from db sources
    tables_to_extract = ['public.address',
                         'public.address_status',
                         'public.author',
                         'public.book',
                         'public.book_author',
                         'public.book_language',
                         'public.country',
                         'public.cust_order',
                         'public.customer',
                         'public.customer_address',
                         'public.order_history',
                         'public.order_line',
                         'public.order_status',
                         'public.publisher',
                         'public.shipping_method']
    
    def requires(self):
        pass
    
    def run(self):
        try:
            
            # Configure logging
            logging.basicConfig(
                filename = f'{DIR_TEMP_LOG}/logs.log',
                level = logging.INFO,
                format = '%(asctime)s - %(levelname)s - %(message)s'
            )
            
            # Define db connection engine
            src_engine, _ = db_connector()
            
            # Define the query using the SQL content
            extract_query = read_sql_file(
                file_path = f'{DIR_EXTRACT_QUERY}/extract_data.sql'
            ) 
            
            start_time = time.time()  # Record start time
            logging.info("==================================STARTING EXTRACT DATA=======================================")
            
            for index, table_name in enumerate(self.tables_to_extract):
                try:
                    
                    # Read SQL file into DataFrame
                    df = pd.read_sql_query(extract_query.format(table_name = table_name), src_engine)
                    
                    # Write DataFrame into CSV 
                    df.to_csv(f'{DIR_TEMP_DATA}/{table_name}.csv', index = False)
                    
                    logging.info(f"EXTRACT '{table_name}' - SUCCESS.")

                except Exception:
                    logging.error(f"EXTRACT '{table_name}' - FAILED.")  
                    raise Exception(f"Failed to extract '{table_name}' tables")
            
            logging.info(f"EXTRACT ALL TABLE FROM SOURCE DATABASE - SUCCESS!")
            
            end_time = time.time()  # Record end time
            execution_time = end_time - start_time  # Calculate execution time
            
            
            # Get summary data
            summary_data = {
                'timestamp': [datetime.now()],
                'task' : ['Extract'],
                'status' : ['Success'],
                'execution_time' : [execution_time]
            }
            
            # Get success summary DataFrame
            summary = pd.DataFrame(summary_data)
            
            # Write success summary DataFrame into CSV
            summary.to_csv(f'{DIR_TEMP_DATA}/extract_summary.csv', index = False)
            
        except Exception:
            logging.info(f"EXTRACT ALL TABLE FROM SOURCE DATABASE - FAILED")
            
            # Get failed summary data
            summary_data = {
                'timestamp' : [datetime.now()],
                'task' : ['Extract'],
                'status' : ['Failed'],
                'execution_time' : [0]
            }
            
            # Get failed summary DataFrame
            summary = pd.DataFrame(summary_data)
            
            # Write failed summary DataFrame into CSV
            summary.to_csv(f'{DIR_TEMP_DATA}/extract_summary.csv', index = False)
            
             # Write exception
            raise Exception(f"FAILED TO EXECUTE EXTRACT TASK !!!")
        
        logging.info("==================================ENDING EXTRACT DATA=======================================")
        
    def output(self):
        
        outputs = []
        
        # Extracted data in CSV
        for table_name in self.tables_to_extract:
            outputs.append(luigi.LocalTarget(f'{DIR_TEMP_DATA}/{table_name}.csv'))
         
        # Extract task summary    
        outputs.append(luigi.LocalTarget(f'{DIR_TEMP_DATA}/extract_summary.csv'))
        
        # Extract task log
        outputs.append(luigi.LocalTarget(f'{DIR_TEMP_LOG}/logs.log'))
        
        return outputs

# # Run the task        
# if __name__ == '__main__':
#     luigi.build([Extract()])