...
if __name__ == '__main__':
    ...
    extract_summary = pd.read_csv(f'{DIR_TEMP_DATA}/extract_summary.csv')
    ...
    concat_df(
        ...
        df2 = pd.read_csv(f'{DIR_TEMP_DATA}/load_summary.csv')
    )
    
    # Concat temp load summary to final summary
    concat_df(
        df1 = pd.read_csv(f'{DIR_ROOT_PROJECT}/pipeline_summary.csv'),
        df2 = pd.read_csv(f'{DIR_TEMP_DATA}/transform_summary.csv')
    )
